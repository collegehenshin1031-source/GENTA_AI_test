...
- 統計表示のスマート化
- 【Step 2】ハゲタカ診断エンジン（AI判定・チャート）の完全統合
"""
...
def show_login_page():
    ...
    with col2:
        ...
        if st.session_state.get("login_error"): st.error("❌ 認証に失敗しました。入力内容をご確認ください。")
...
def show_main_page():
    if not st.session_state.get("disclaimer_agreed") and st.session_state.get("login_type") != "master":
        # Note: Added fallback for older sessions, logic moved to login button. 
        pass 
    
    logo_base64 = get_logo_base64()
    if logo_base64: st.markdown(f'<div style="text-align: center; margin-bottom: 0.5rem;"><img src="data:image/png;base64,{logo_base64}" class="logo-img" style="max-width: 320px; width: 80%;"></div>', unsafe_allow_html=True)
    else: st.title("🦅 HAGETAKA SCOPE")
